...
export class LinkEntity {
  ...
  protected async _negotiateClaim(setTokenRenewal?: boolean): Promise<void> {
    ...
    const tokenObject = await this._context.tokenProvider.getToken(
      this.audience
    );
    log.link(
  ...
  protected async _ensureTokenRenewal(): Promise<void> {
    const tokenValidTimeInSeconds = this._context.tokenProvider
      .tokenValidTimeInSeconds;
    const tokenRenewalMarginInSeconds = this._context.tokenProvider
      .tokenRenewalMarginInSeconds;
    const nextRenewalTimeout =
      (tokenValidTimeInSeconds - tokenRenewalMarginInSeconds) * 1000;
